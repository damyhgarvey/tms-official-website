...
async function loadCatalog(){
  try{
    ...
    root.innerHTML=data.offers.map(o=>`<article class="card"><span class="num">${esc(o.offer_id)}</span><div class="icon">✦</div><h3>${esc(o.name)}</h3><p>${esc(o.category)} • ${money(o.price_mga)}</p><p>Marge indicative : ${money(o.margin_mga)} • ROI : ${esc(o.roi_pct)}%</p><span class="status"><span></span>DISPONIBLE</span><a href="request.html">Demander cette offre →</a></article>`).join('');
